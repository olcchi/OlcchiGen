...
export default function MemoryFadePage() {
  ...
  useEffect(() => {
    ...
    const loadLibraries = async () => {
      ...
      const sketch = (p: any) => {
        // Text content
        const text = "因为不知死何时将至，我们仍将生命视为无穷无尽、取之不竭的源泉。然而，一生所遇之事也许就只发生那么几次。曾经左右过我们人生的童年回忆浮现在心头的时刻还能有多少次呢？也许还能有四五次。目睹满月升起的时刻又还能有多少次呢？或许最多还能有二十次。但人们总是深信这些机会将无穷无尽。"
        let characters: any[] = []
        let floatingChars: any[] = []
          ...
          for (let i = 0; i < text.length; i++) {
            ...
            const shouldBreak =
              ...
              (currentLine.length > maxCharsPerLine * 0.8 &&
                (char === '的' || char === '了' || char === '着' || char === '过')) // Common word endings
        ...
        const triggerCharacterFloat = (charIndex: number) => {
          ...
          char.body = Bodies.rectangle(char.x, char.y, charWidth, charHeight, {
            ...
            frictionAir: 0.03, // 增加空气阻力，让移动更缓慢
        ...
        p.draw = () => {
          ...
          if (isFloating && floatQueue.length > 0) {
            ...
            if (remainingNonMemoryChars.length === 0) {
              ...
              const baseCount = 2
              const progressiveCount = Math.min(13, Math.floor(floatCount * 0.8)) // 随时间增加，最多13个
              numToFloat = Math.min(remainingNonMemoryChars.length, baseCount + progressiveCount)
            }
          ...
          if (!isFloating && !isResetting && allFloatedTime > 0) {
            ...
            if (timeSinceAllFloated > 2000) { // 2秒后开始重置
              startReset()
            }
          }
          ...
          if (isResetting && resetQueue.length > 0) {
            ...
            if (timeSinceLastReset > p.random(50, 200)) { // 随机间隔50-200毫秒
              const numToReset = Math.min(resetQueue.length, Math.floor(p.random(1, 4))) // 1-3个字符
          ...
          for (let char of floatingChars) {
            if (char.body) {
              ...
              // 每个字符有自己的风向和强度（主要向右）
              const personalWindX = Math.cos(char.windDirection) * baseWindForce * char.windStrength
              const personalWindY = Math.sin(char.windDirection) * baseWindForce * char.windStrength * 0.5 // 垂直方向减半
              ...
              // 很小的随机扰动
              const randomWindX = (p.noise(char.index * 0.1, p.millis() * 0.0002) - 0.5) * 0.000002
              const randomWindY = (p.noise(char.index * 0.1 + 100, p.millis() * 0.0002) - 0.5) * 0.000002
  ...
  return (
    <>
      ...
      <div className="w-120 h-120 max-w-[90%] border border-black">
